...
"""
2018-05-14, v2 RW
    - TODO: change to AI class from Virginia in purpose that voltage monitors can run in parallel to the fluoresence scan program

2018-04-19, v2 RW
    - checked why voltage/current monitor is so slow
        - main reason is current readback via polling ISEG, about 0.8s for all 8 channels, but not investigated if can be done more time efficient
        - voltage readback ia about 0.2s for 8 channels. checked AI reading with nidaqmx_caller class from Virginia, about 0.02s perchannel, so no big difference
        - writing to file is less then 0.1s
    - uncommented current readback for now, to acitave it search for "CurrentReadback__list = self.trap_control.iseg_modules.measure_currents()"

    2018-03-02, v2 RW
    - loads Penning_trap_cxn_dictionaries_v2 which includes all trap electrodes and no egun stuff
    - LTCE1 and STEC2 voltage readbacks have to be calibrated
    
2018-01, v1 CM, CT
    - removed egun tab in create_TabWidget
...
from Penning_trap_cxn_dictionaries_v2 import trap_surf_dict, iseg_controlled_trap_surfaces, egun1_surf_dict
from voltage_configuration_dictionary_v2 import volt_config_dict

from instrument_command import NI_USB_6259_Command

import egun1_module
import trap_surfaces_module_v2
...
#=================================================================================================
    ...
    def setup_control_system(self):    
        ...
        self.trap_control = trap_surfaces_module_v2.trap_control()
...
#        self.create_tab_egun1()
        ...
        self.create_TabWidget([self.tab_Vinitial, self.tab_shuttling, self.tab_Vfinal, self.tab_monitor_trap_voltages], ['Initial/Loading Voltages','Ion Shuttling','Final Trap Voltages', 'Monitor Trap Voltages'])
...
#=================================================================================================
    ...
    def NIDAQ_MON_and_REC__shuttling_sequence(self):  
        ...
        while self.running:
            ...
            #print(trap_surf_dict[self.trap_surfaces[i]]['NIDAQ_AI'])
        ...
        while self.running:
            ...
            if True:
                ...
                MeasuredSignal__monitor_trap_voltages_all_dec = [self.nidaq.measure_AI(AI=trap_surf_dict[self.trap_surfaces[i]]['NIDAQ_AI']) * \
                                   trap_surf_dict[self.trap_surfaces[i]]['NIDAQ_2_HV_CONVERSION'] for i in range(self.num_trap_surfs)]
                #toc = time.time()
                #print ("voltagereadback", toc-tic, "s")
                ...
                ####################################################################################
                ######################## uncomment for current readback ############################
                #CurrentReadback__list = self.trap_control.iseg_modules.measure_currents()
                CurrentReadback__list = [0,0,0,0,0,0,0,0]
                ####################################################################################
                ####################################################################################

                #print CurrentReadback__list
                #toc = time.time()
                #print ("currentreadback", toc-tic, "s")
